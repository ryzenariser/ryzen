...
async function runGeminiWithTools(systemInstruction, contents, tools, session) {
...
async function handleAssistant(req, res) {
  ...
  const finalReply = await runGeminiWithTools(systemInstruction, contents, tools, session);
...
async function handleTelegramWebhook(req, res) {
  ...
  try {
    ...
    const reply = GEMINI_API_KEY
      ? await runGeminiWithTools(systemInstruction, contents, tools, { sub: null })
  ...
  for (const row of activeRoles || []) {
    ...
    try {
      ...
      const geminiRes = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ system_instruction: systemInstruction, contents: [{ role: 'user', parts: [{ text: 'Give today\'s brief.' }] }] }),
      });
...
async function sendToCmoBot(text) {
  const token = process.env[`TELEGRAM_BOT_TOKEN_${ROLE_ENV_KEYS.CMO}`];
  if (!token) return;
  const { data: botRow } = await supabase.from('telegram_role_bots').select('chat_id').eq('role_title', 'CMO').maybeSingle();
  if (!botRow?.chat_id) return;
  await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: botRow.chat_id, text }),
  }).catch((err) => console.error('sendToCmoBot failed:', err.message));
}
...
async function handleCronWeeklyStrategy(req, res) {
  ...
  try {
    ...
    const geminiRes = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ system_instruction: systemInstruction, contents: [{ role: 'user', parts: [{ text: 'Generate the strategy.' }] }] }),
    });
    if (!geminiRes.ok) return res.status(200).json({ ok: false, reason: `Gemini ${geminiRes.status}` });
    const data = await geminiRes.json();
    const text = data.candidates?.[0]?.content?.parts?.map((p) => p.text || '').join('') || '';
...
async function handleBrandVoiceCheck(req, res) {
  ...
  const geminiRes = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ system_instruction: systemInstruction, contents: [{ role: 'user', parts: [{ text }] }] }),
  });
  if (!geminiRes.ok) {
    return res.status(502).json({ error: `AI service error (${geminiRes.status}). Try again in a moment.` });
  }
