...
async function handleDashboard(req, res) {
  ...
  const { data: admin } = await supabase
    .from('admins')
    .select('deactivated_at, org_title, username')
    .eq('id', session.sub)
    .maybeSingle();
...
async function handleAssistant(req, res) {
  ...
  const systemInstruction = {
    parts: [
      {
        text:
          'You are the admin assistant inside the Razariser store\'s admin panel — operating under the AI Support ' +
          'function of Razariser\'s Chief AI & Innovation Officer (CAIO) role per the org chart. ' +
  ...
  const systemInstruction = {
    parts: [{
      text:
        ...
        'execute manually. Base it on the real store context below. Razariser has no ad platform, email/SMS provider, ' +
