...
/* ── Local biometric face comparison (runs in-process, no external API) ── */
const { verifyFaceLocally } = require('./faceMatch');
...
module.exports = async (req, res) => {
  try {
    ...
    if (action === 'face-verify') {
      ...
      let result;
      try {
        result = await verifyFaceLocally(referenceImage, imageBase64);
      } catch (err) {
