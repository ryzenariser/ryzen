...
function marketingContextToPromptText(context) {
  ...
  return [
    ...
    context.orders.hasData
      ...
      : `Orders: NONE YET — Ryzen has no completed sales history. Do not invent conversion rates, revenue figures, CAC, LTV, or forecasts. State plainly that these require real order data once sales start.`,
  ].join('\n');
}
...
async function handleMarketingBrief(req, res) {
  ...
  const systemInstruction = {
    parts: [{
      text:
        'You are the Marketing Director AI for Ryzen, a premium men\'s pre-order fashion brand. You are writing ' +
  ...
  const geminiRes = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ system_instruction: systemInstruction, contents: [{ role: 'user', parts: [{ text: 'Generate today\'s brief.' }] }] }),
  });
...
async function handleMarketingContent(req, res) {
  ...
  const toneLabel = TONE_LABELS[tone] || 'Confident, premium, understated (Ryzen\'s default voice)';

  const systemInstruction = {
    parts: [{
      text:
        'You are the Marketing Director\'s AI Content Studio inside the Ryzen store\'s admin panel. ' +
        'Ryzen is a premium men\'s fashion brand (T-shirts, cargo pants, joggers, co-ords) sold via pre-order, ' +
        'with a black-and-gold premium aesthetic. ' +
        `Tone for this piece: ${toneLabel}. ` +
        `Generate THREE distinct variants of ${typeLabel} based on the admin's brief — different angles or hooks, ` +
        'not just reworded copies of each other. These are DRAFTS for the admin to review and edit — never claim ' +
        'they have been published or sent anywhere. Do not invent specific statistics, discount percentages, or ' +
        'claims not given to you. If the brief references a specific product, use the real product context below ' +
        'if it matches.\n\n' +
  ...
  const geminiRes = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ system_instruction: systemInstruction, contents: [{ role: 'user', parts: [{ text: userText }] }] }),
  });
...
async function handleMarketingCampaign(req, res) {
  ...
  const systemInstruction = {
    parts: [{
      text:
        'You are the Marketing Director AI for Ryzen, a premium men\'s pre-order fashion brand. Draft a campaign ' +
        'PLAN — this is never auto-published or auto-scheduled, it is a proposal for the admin to review and ' +
        'execute manually. Base it on the real store context below. Ryzen has no ad platform, email/SMS provider, ' +
