...
async function handleFaceChallengeVerify(req, res) {
  ...
  let matchResult = await compareFacesWithGemini(ownReferenceBuffer, challengeBuffer);
  let matchedAs = 'self';
  ...
  // stand in and unlock any sub-admin's login this way.
  if (!matchResult.match && admin.role !== 'super_admin') {
    ...
    if (owner && owner.face_reference_path) {
      ...
      if (!ownerDlError) {
        ...
        const ownerMatch = await compareFacesWithGemini(ownerReferenceBuffer, challengeBuffer);
        if (ownerMatch.match) {
  ...
  await sendFaceChallengeToTelegram(admin, req, matchResult, challengeBuffer, matchedAs);
